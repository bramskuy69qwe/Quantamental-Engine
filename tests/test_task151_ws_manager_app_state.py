...
# ── Functional FBF: simulate a call path that hits an app_state read ───────

class TestUserDataLoopFunctionalPath:
    """End-to-end FBF: drive `_user_data_loop` down a path that
    reaches an `app_state` reference, confirm no UnboundLocalError.
    Pre-fix this would raise; post-fix it executes cleanly (or
    raises a different exception that we ignore — we're only
    testing the UnboundLocalError shape)."""

    @pytest.mark.asyncio
    async def test_call_with_plugin_connected_does_not_raise_unbound_local(
        self, monkeypatch,
    ):
        """Path: platform_bridge.is_connected=True → enters the
        early-return branch at line 415. That branch reads
        `app_state.ws_status.add_log(...)`. Pre-fix: UnboundLocalError.
        Post-fix: the asyncio.sleep(30) is what we hit next; cancel
        before it completes so the test doesn't hang."""
        import asyncio
        from core import ws_manager
        from unittest.mock import MagicMock

        # Mock platform_bridge.is_connected = True
        mock_pb = MagicMock()
        mock_pb.is_connected = True
        monkeypatch.setattr(
            "core.platform_bridge.platform_bridge", mock_pb, raising=False,
        )

        # Make _stopping True so the inner reschedule doesn't recurse
        monkeypatch.setattr(ws_manager, "_stopping", True, raising=False)

        # Patch asyncio.sleep to a fast-no-op so the test completes
        async def _fast_sleep(_seconds):
            return None
        monkeypatch.setattr(ws_manager.asyncio, "sleep", _fast_sleep)

        # Run the loop — must not raise UnboundLocalError
        try:
            await ws_manager._user_data_loop("test_listen_key", attempt=0)
        except UnboundLocalError as exc:
            pytest.fail(
                f"FE-MED-034 regression: UnboundLocalError raised by "
                f"_user_data_loop: {exc}. Indicates the inline import "
                f"shadow has been re-introduced."
            )
        except Exception:
            # Any other exception is fine — we're only testing the
            # UnboundLocalError shape. Function may legitimately raise
            # other errors (e.g., websockets.connect failure) when
            # exercised without a real WS server.
            pass

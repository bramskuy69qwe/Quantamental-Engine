// QE Shared — design tokens + base atoms
const QEC = {
  bg:        '#07080f',
  card:      '#0c1118',
  panel:     '#101826',
...
const Mono = ({children, color, size='0.95rem', style={}}) => (
...
const Stat = ({label,value,color,size,sub}) => (
  <div>
    <Lbl>{label}</Lbl>
...
const Card = ({children, style={}, p='10px 12px'}) => (
...
const Badge = ({children, color=QEC.text, bg=QEC.muted, style={}}) => (
...
const Btn = ({children, variant='secondary', size='md', onClick, style={}, disabled}) => {
  const variants = {
...
const Sparkline = ({data=[], color=QEC.green, height=60, showFill=true, gridLines=4}) => {
...
function mockEquityData(n=60, base=82.2, vol=3) {
  let v = base, data = [];
  for (let i = 0; i < n; i++) {
...
// ── TabBar — shared, 4 style variants ────────────────────────────────────
// variant: 'line' | 'filled' | 'pill' | 'bracket'
const TabBar = ({tabs, active, onChange, variant='line', style={}}) => {
...
Object.assign(window, {
  QEC, REGIME_META,
  Lbl, Mono, Stat, Card, SecLabel, Badge, OkBadge, Divider, Inp, Sel, Btn,
  Sparkline, mockEquityData, fmtPnl, pnlColor,
  TabBar,
});

...
const TopNav = ({ page, setPage, platform, setPlatform, pluginConnected }) => {
    ...
    <header style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
        ...
        <select value={platform} onChange={(e) => setPlatform(e.target.value)} style={{
          background: QEC.panel, border: `1px solid ${QEC.border}`, color: QEC.text,
          borderRadius: '4px', padding: '2px 6px', fontSize: '0.68rem', height: '24px', cursor: 'pointer', width: '110px', fontFamily: "\"Space Grotesk\""
        }}>
          <option value="standalone">Standalone</option>
          <option value="quantower">Quantower</option>
        </select>

        {/* Plugin badge — Quantower only */}
        {platform === 'quantower' &&
        <div style={{ display: 'flex', alignItems: 'center', gap: '3px', fontSize: '0.65rem', fontFamily: 'JetBrains Mono,monospace', whiteSpace: 'nowrap' }}>
            <span style={{ color: pluginConnected ? QEC.green : QEC.muted }}>
              {pluginConnected ? '●' : '◌'}
            </span>
            <span style={{ color: pluginConnected ? QEC.green : QEC.muted, fontWeight: 700 }}>Plugin</span>
          </div>
        }
        ...
        {platform === 'quantower' &&
        <span style={{
          fontSize: '0.65rem', padding: '2px 7px', borderRadius: '99px', fontWeight: 700, fontFamily: 'JetBrains Mono,monospace',
          background: pluginConnected ? '#071a10' : '#251500',
          color: pluginConnected ? QEC.green : QEC.amber
        }}>
            {pluginConnected ? '● Live' : '◌ Offline'}
          </span>
        }

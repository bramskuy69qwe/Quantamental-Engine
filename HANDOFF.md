...
4. **#3c — NEW follow-up (filed by task 305's audit; pre-existing, NOT a regression).** Same hx-boost re-execution root cause as #3a but for `addEventListener`, not `setInterval`: several body-level IIFEs re-register `document.body.addEventListener(...)` on every boosted nav. Because hx-boost swaps the body's **innerHTML** (the `<body>` NODE persists), these listeners ACCUMULATE one copy per navigation — sites: `htmx:responseError`/`htmx:sendError` (the user-visible one — N navs → N duplicate error toasts + N target-swaps), the steppers `htmx:afterSettle`, the dashTab + hpTab `htmx:afterSettle`, and `account-added`. Out of #3a's stated scope (which named only the two `setInterval` poll IIFEs). Fix: guard each IIFE's `addEventListener` block behind a `window._X` flag (same partial-guard nuance as the connection-poll — guard the listener registration, keep any per-nav setup like the initial `attachSteppers()` call running). **User-visible severity (the case for prioritizing this):** the stacking `htmx:responseError`/`htmx:sendError` handlers mean every still-non-2xx path — e.g. `calculate_risk`'s 400 validation bodies, which #3b did NOT convert to 200 — fires N duplicate error toasts + N target-swaps after N boosted navs. NB the notif IIFE's comment ("body is replaced by the swap") is subtly wrong — the body node persists; that misframing is what hid this listener-stacking class.

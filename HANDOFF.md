...
**Date**: 2026-07-16 (**v2.6 QUANTOWER-PLUGIN REMOVAL is COMPLETE** — all 6 phases shipped, per-phase audited, suite green. **Next session = AUDIT + DEBUG the v2.6 work** — see ▶ NEXT SESSION.)
**Branch**: **`v2.6/remove-quantower-plugin`** — **ahead of origin by 3** (Phases 1-3 PUSHED; **P4 `523e0d1`, P5 `aeb397f`, P6 `8efce34` UNPUSHED** — push is the operator's call). Forked off `3a55f3e` (the `v2.5/usertrades-backfill-fix` tip). Verify `git status -sb` at session start.
**Tests**: full suite **3939 passed / 7 skipped / 3 deselected / 0 xfailed** — green (~5:20). Run SOLO. ⚠ One run this session hit a **transient `concurrent_log_handler` flush timeout** (0 lingering pythons after; re-run clean) — the CLAUDE.md test-hang class; re-run once before treating it as real.
...
## ▶ STATUS 2026-07-16 — v2.6 QUANTOWER-PLUGIN REMOVAL COMPLETE (Phases 1-6)
...
- **Landmines KEPT (plan §2 — do NOT "clean" these):** **L2** `resolve_tpsl_direction` (`order_state.py`, used live by exchange/ws_manager/order_manager); **L3** `UpdateSource.PLATFORM` + `data_cache.apply_account_update_platform` (now dead-but-retained — deleting = wide test rewrite for no gain); **L4** historical `source='quantower'` fills + `scripts/dedup_fills.py "quantower": 0` + `test_dedup_fills`/`test_an2_quantower_cleanup`/`migrations/000_split_databases.py`; **L5** `/api/backtest/qt-import` + its UI in `templates/backtest.html`/`fragments/backtest/results.html` (the Quantower BACKTEST-RESULTS importer — a file upload, NOT the plugin; retire in v2.7).
- **Named per-phase deviations (full text in commit bodies):** P4 deferred tasks 4.2 (`PLATFORM_TOKEN`) + 4.3 (QT maps) to P5 — both consumed only by the P5-deleted modules, and the QT maps' MODULE-LEVEL import in `platform_bridge` would break boot (via `api/router.py → routes_platform → platform_bridge`) if removed early. P2 kept `monitoring._check_plugin_connection_sync` (folded read→False) + its 3 tests rather than deleting. Obsolete plugin tests were removed per-phase (P2 standby regression; P4 7 plugin-UI tests in test_task121/time_sync/phase8_deferred; P5 `TestPlatformFrames` + `TestNoDbConnInExecutingCode`).
- **DEFERRED / opportunistic (non-blocking):** the corr-log `platform_*` categories (`CAT_PLATFORM_FILL/SNAPSHOT/HELLO/PUSH` in `core/correlation_log.py`) are now **producer-less dead-but-present** — removing them changes the snapshot-pinned category count (registry is 46 categories), so it's a SEPARATE corr-log cleanup (update the spec + the pinned count together).

## ▶ NEXT SESSION — AUDIT + DEBUG the v2.6 work

**Mission (operator-set 2026-07-16): independently audit + live-debug the v2.6 removal before it's pushed.** Each phase was already 2-agent-audited CLEAN, but the operator wants a holistic pass + real-engine debugging. Suggested scope (ultracode is on — a Workflow fan-out fits the grep/exercise/verify shape):

1. **Grep-proof completeness** (CLAUDE.md post-migration discipline): re-grep `platform_bridge`, `routes_platform`, `PLATFORM_TOKEN`, `active_platform`, `QT_STATUS_MAP`, `QT_ORDER_TYPE_MAP`, `/ws/platform`, `/api/platform`, `Quantower`/`quantower`. Every remaining hit must be a KEPT landmine (L2-L5), a historical record (docs/past, docs/audits, docs/design, CHANGELOG, HANDOFF), a dead corr-log category, or a comment about the Quantower TRADING PLATFORM the engine OBSERVES (observe-only architecture — accurate, not stale). Flag anything that describes the removed RISK PLUGIN as if it were live. (P6 already swept README/config.md/CLAUDE.md/AGENTS.md/binance.md/handlers/exchange_income/ws_manager/order_manager/link_actions/accounts.html/primitives — verify no miss remains.)
2. **Boot-path audit — the class our smokes MISSED:** every per-phase boot smoke did only `import main` (module load), which does NOT run the async startup lifespan that connects to the exchange. A runtime startup regression could hide there. **Live-debug the full startup** on the running engine (operator drives, clock synced): confirm it reaches `Background startup complete — … fully ready`, WS connects (`ws_connected stream=user`), and the observe-only fill→position→closed_position→calc-linkage path works exchange-only. Use `CORR_LOG_PROFILE=linkage` + `scripts/corr_tail.py` (cookbook).
3. **Exchange-only regression sweep:** exercise the paths the plugin used to touch, now singleton/exchange-only — WS `ORDER_TRADE_UPDATE`/`ACCOUNT_UPDATE`/`ALGO_UPDATE` ingestion (`order_manager.process_order_update` via the singleton), TP/SL enrich (`fetch_open_orders_tpsl`), OHLCV/depth, income backfill, dashboard/history `open_orders` (now `order_manager_singleton.order_manager`). All must work with NO plugin.
4. **Startup REST-throttling improvement (optional, NOT a v2.6 bug):** on a stale/long-offline snapshot the startup's heavy backfills (`fetch_exchange_trade_history` income paging + `recover_offline_trades days=90` + per-position `fetch_ohlcv`, all at `_startup_fetch` in `core/schedulers.py:505-545`) run BEFORE `is_initializing=False`, so a rate-limited or clock-drifted start blocks "ready" for minutes. Candidate fix: flip the ready flag first, defer those to background. Own task, operator-gated.
...
## Correlation-log program state — COMPLETE: Phases 0–5, all 12 tasks (TEN §5.6 attribution categories live; 46-category registry) — PUSHED
...
**Where things live**: `core/correlation_log.py` (registry/_PROFILES/emit/sink — the spine; **46 categories** incl. 10 attr, snapshot-pinned). Taps: `main.py`, `core/{ws_manager,platform_bridge,news_fetcher,event_bus,webhook_dispatcher,regime_fetcher,schedulers,monitoring,data_cache,calc_state,link_state,order_manager,order_enrichment,calc_correlation,link_actions,db_orders,db_trades,event_log,trade_event_log,funding_handler}.py`, `core/adapters/base.py`, `core/pubsub/*`. **Reader**: `scripts/corr_tail.py` + `docs/correlation_log_cookbook.md`. Conftest: `_corr_log_session_floor` (SESSION) + `_isolate_correlation_log_dir` + per-test corr reset. Tests: `tests/test_correlation_log_{spine,entrypoints,ws,bus,boundaries,floor,state,attribution,replay,perf}.py` + `test_corr_tail.py` (~310 corr tests; `perf` excluded from default). Governing docs: `docs/design/correlation_log_spec.md` (§15 errata E1–E34; status COMPLETE) + `docs/design/correlation_log_implementation_plan.md` (§9.2 SHIPPED shas, §9.4 ledger HA-1..42 with full Phase-5 disposition).

...
/* Static demo equity series for the Primitives proving-ground charts —
   deterministic (no mock RNG in the render path). */
const DEMO_EQUITY = [82.20, 82.44, 82.10, 82.86, 83.40, 83.02, 83.95, 84.60, 84.18, 85.22, 85.90, 85.55, 86.40, 87.10, 86.72, 87.85, 88.40, 88.05, 89.10, 89.95];
...
data: 93580.40`}</pre>
        ...
        <Card pad>
          ...
          <div className="spec-row">
            ...
            <div style={{width:'100%'}}>
              <HeatStrip height={20} data={Array.from({length:30},(_,i)=>({date:`04-${String(i+1).padStart(2,'0')}`, pnl:+((Math.sin(i*1.7)+Math.cos(i*0.5))*0.9).toFixed(2)}))}/>
            </div>
          </div>
          <div className="spec-row">
        ...
        {/* Workspace + ECharts — the P0 proving ground for GridStack↔React and
            the ECharts theme (the two P0 acceptance items). */}
        <Card pad style={{gridColumn:'1 / span 2', borderColor:'var(--qe-cyan)'}}>
          <SecLbl rule>Workspace · GridStack tiling in React + ECharts theme</SecLbl>
          <div style={{fontSize:'0.62rem', color:'var(--qe-sub)', marginBottom:8, lineHeight:1.55}}>
            <code style={{color:'var(--qe-cyan)'}}>GridWorkspace</code> owns the gs-* attributes imperatively, so React
            re-renders never reset the layout. <strong style={{color:'var(--qe-text)'}}>Drag</strong> a pane by its title bar,
            <strong style={{color:'var(--qe-text)'}}> resize</strong> from any edge; the top-nav <strong style={{color:'var(--qe-text)'}}>lock</strong> (🔓) freezes every workspace.
            The Equity + Sparkline panes render through <code style={{color:'var(--qe-cyan)'}}>QE_ECHARTS_THEME</code> (canvas, vendored ECharts).
          </div>
          <div style={{display:'flex', flexDirection:'column', height:248, border:'1px solid var(--qe-line)'}}>
            <GridWorkspace cols={12} rows={12} persistId="primitives-demo">
              <GridItem x={0} y={0} w={7} h={7}>
                <Pane title="Equity" tag="ECHARTS" foot={{tone:'ok', id:2101, msg:'demo series · QE_ECHARTS_THEME', ms:3}}>
                  <EquityChart data={DEMO_EQUITY} baseline={DEMO_EQUITY[0]}/>
                </Pane>
              </GridItem>
              ...
              <GridItem x={0} y={7} w={12} h={5}>
                <Pane title="Sparkline" hot foot={{tone:'info', id:2102, msg:'drag my title bar · resize my edges', ms:0}}>
                  <div style={{padding:'10px 8px'}}><Sparkline data={DEMO_EQUITY} height={44} color="var(--qe-cyan)"/></div>
                </Pane>
              </GridItem>
            </GridWorkspace>
          </div>

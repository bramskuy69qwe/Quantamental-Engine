...
import { QEC, REGIME_META, mockEquityData, Card, SecLabel, Lbl, Mono, Stat, Badge, Btn, Divider, Sparkline, Inp, Sel, TabBar } from './qe-shared.jsx';
...
function buildSegments(data) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i].regime === cur.regime) { cur.end = i; cur.days++; }
    else { segs.push(cur); cur = { regime: data[i].regime, start: i, end: i, days: 1 }; }
  }
  segs.push(cur);
  return segs;
}
...
const TLBars = ({ data }) => {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: `${H}px`, display: 'block', borderRadius: '3px', overflow: 'hidden' }}>
      {data.map((d, i) =>
        <rect key={i} x={i * bw} y={0} width={Math.max(bw, 0.5)} height={H} fill={TL_COLOR[d.regime] || '#4a6888'} opacity="0.9"/>
      )}
    </svg>
  );
};
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <svg viewBox={`0 0 1000 ${REGIME_ORDER.length * (LANE + GAP)}`}
          ...
          {REGIME_ORDER.map((r, li) =>
            <rect key={r} x={0} y={li * (LANE + GAP)} width={1000} height={LANE} fill={TL_COLOR[r]} opacity="0.07"/>
          )}
          {segs.map((s, i) => {
    ...
    <div style={{ overflowX: 'auto' }}>
      <svg viewBox={`0 0 ${weeks * (CELL + GAP)} ${rows * (CELL + GAP)}`}
        ...
        {cells.map((d, i) => {
          ...
          return (
            <rect key={i} x={x} y={y} width={CELL} height={CELL} rx="2" fill={c} opacity={d ? 0.88 : 0.3}/>
          );
        })}
      </svg>
    </div>
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: `${H}px`, display: 'block' }}>
      {buckets.map((b, bi) => {
        ...
        return REGIME_ORDER.map((r) => {
          ...
          y -= h;
          return <rect key={r} x={bi * BW} y={y} width={Math.max(BW - 0.5, 0.5)} height={h} fill={TL_COLOR[r]} opacity="0.88"/>;
        });
      })}
    </svg>
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr', gap: '6px' }}>
        <Card p="10px 12px" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
          ...
          <Badge color={rm.text} bg={rm.bg} style={{ fontSize: '0.78rem', padding: '5px 12px', letterSpacing: '0.06em' }}>Neutral</Badge>
            ...
            {Object.entries(REGIME_DIST).map(([key, pct]) => {
              ...
              return (
                <div key={key} style={{ textAlign: 'center', background: `${m.bg}`, borderRadius: '4px', padding: '6px 4px' }}>
                  ...
                  <div style={{ fontSize: '0.58rem', color: QEC.sub, marginTop: '1px' }}>{Math.round(pct * 3.65)}d</div>
                </div>
              );
            })}
          </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', padding: '8px' }}>
      ...
      <Card>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr>
            {['Name','Exchange','Type','Status','Actions'].map((h) =>
              <th key={h} style={{ color: QEC.sub, fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase',
                padding: '5px 8px', borderBottom: `1px solid ${QEC.border}`, textAlign: 'left' }}>{h}</th>
            )}
          </tr></thead>

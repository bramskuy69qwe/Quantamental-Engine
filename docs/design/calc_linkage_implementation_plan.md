...
| 4.6 | **SHIPPED (P4.T5, 2026-06-02)**: populate `closed_positions.tp_drift_pct` / `sl_drift_pct` = (final amended TP/SL − planned TP/SL) / planned × 100. Computed in `_compute_close_deltas` (the T2.5 delta home). **planned_tp/sl come from the PRIMARY (most-contributing) calc's junction snapshot — NOT literally the "first" calc** (spec §3.2 delta-basis rule; consistent with every other close-row delta + the T2.6 convergence). **final_tp/sl = the latest `order_amendments.new_value` for `tp_price`/`sl_price` on the primary calc's legs** (reuses `get_calc_amendments`, last-wins per field) — the orders row goes stale post-amendment so the ledger is authoritative (P4.T1). **Amended-only**: NULL when the leg was never amended (the §4 acceptance criterion "populated for amended-stop positions") or planned is missing. Persisted + REPLACE-preserved: added to `_CLOSED_POS_DELTA_COLS` AND to `insert_closed_position`'s INSERT column/placeholder lists (the schema had the columns but the INSERT never wrote them — the gap the real-close-path test caught). **Independent audit (6 agents): 1 MED fixed** — SCOPING-001: a protective leg amended BEFORE bracket inheritance assigned its calc_id left an orphaned `calc_id=NULL` amendment that the calc_id-scoped drift (and P4.T2's count) silently missed; fixed at the root in `_propagate_bracket_calc_id` (`_apply_leg` now backfills `UPDATE order_amendments SET calc_id WHERE order_id=? AND calc_id IS NULL` alongside the orders update — covers WS + REST). **Residual documented edge**: a scale-in protective leg inherited under a non-primary calc_id (T2.9 earliest-entry pick) → drift NULL on the §3.2 primary basis. Tests: `tests/test_phase4_drift.py` (13) + 2 bracket-backfill tests + 2 refreshed T2.5 deferred-column assertions. | `core/order_manager.py:_compute_close_deltas()` + `_propagate_bracket_calc_id()`, `core/db_orders.py` |
...
| 7.2 | **SHIPPED (task 271)**: `GET /context/position/{position_id}` — full graph keyed on `terminal_position_id` via `assemble_position_context`. Reuses the T1 assembler: extracted a shared `_aggregate_tail` (amendments + funding + position resolution + events) now used by BOTH lifecycle + position (lifecycle unchanged). `position` resolves THIS position open-or-closed (not most-contributing — the position IS the key); works for junction-less / UNPLANNED positions (calcs/amendments/events calc-keyed → empty there). 2 new reads `get_orders_by_position_id`/`get_fills_by_position_id`. Audited (2 agents); tests `test_phase7_context.py` (38). | `core/context_query.py`, `core/db_orders.py`, `api/routes_context.py` |
...
| 7.4 + 7.5 | **SHIPPED (tasks 274-275)**: position-closed webhook dispatcher — the FIRST in-process event_bus `position:closed` subscriber. `core/webhook_dispatcher.py`: per-account handler closures ENQUEUE the §9 payload (non-blocking — the event_bus dispatch loop is sequential, so retrying HTTP stays off it); a background worker POSTs `{event: position_closed, payload: <§9>}` to the account's `webhook_url` with exponential backoff (1→2→4…cap 30s, 5 attempts) + an `engine_events` `webhook_dispatch_failed` dead-letter on exhaustion. `webhook_url` + STRICT-bool `feature_flags.webhook_enabled` on `AccountConfig` (default OFF — no accidental outbound). `json_safe` keeps NaN/Infinity off the wire (and out of the dead-letter record). Wired into `_startup_fetch` via the testable `start_webhook_dispatcher` helper (subscribe-before-run; guarded). **Deviation**: dead-letter is an engine_events ROW, not a replayable QUEUE table — spec §11.3 is fire-and-forget with the subscriber re-fetching via §7.1/7.2, so a miss self-heals (replay = deferred follow-up). Audited (2 agents); tests `test_phase7_webhook.py` (19). | `core/webhook_dispatcher.py` (new), `core/account_config.py`, `core/event_log.py`, `core/schedulers.py` |
| 7.6 | **JSON + PDF SHIPPED (tasks 277-281)**: `POST /export/closed_position/{id}` → a signed audit bundle (`?format=json` default | `?format=pdf`). `core/audit_export.py::build_closed_position_export` resolves the closed row by PK → its `terminal_position_id` → `assemble_position_context` (REUSES the P7.2 assembler → the full §10.6 graph), falling back to the lifecycle graph then a closed-row-only degenerate bundle. Signed-timestamp envelope: signature over canonical(header+bundle) — HMAC-SHA256 if `config.EXPORT_SIGNING_KEY` set (authenticity) else unkeyed SHA-256 (tamper-evidence; localhost default, CLAUDE.md Task 163). `json_safe` keeps NaN/Inf out of signed+served bundle. **PDF (T5)**: `render_export_pdf` + the vendored dependency-free `core/pdf_writer.py` (paginated Courier text + the SAME signature in a footer). `get_closed_position_by_id` (new). Audited (4 agents total; PDF byte-validity verified); tests `test_phase7_export.py` (26). | `core/audit_export.py` (new), `core/pdf_writer.py` (new), `api/routes_export.py` (new), `core/db_orders.py`, `config.py`, `api/router.py` |
| 7.7 | **SHIPPED (tasks 283-284)**: `POST /export/closed_positions?account_id=&from_ms=&to_ms=&format=json\|pdf` → a ZIP of one signed bundle per closed position in the (epoch-ms) range + a signed `manifest.json`. `core/audit_export.py::build_batch_export` reuses `build_closed_position_export`, DEDUPS multi-partial rows by `terminal_position_id` (empty-tpid observe-only rows are each their own member), and signs the manifest over its own canonical form (incl. every member signature → tamper-evident as a whole). `MAX_BATCH_POSITIONS` cap surfaced via `manifest.truncated` (no silent cap). `render_batch_zip` packs via stdlib zipfile (signature on the manifest → zip non-determinism irrelevant). `get_closed_positions_in_range` (new, `idx_closed_pos_ts`). Account-scoped. Audited (2 agents; Agent A 0 findings); tests `test_phase7_export.py` (38). | `core/audit_export.py`, `core/db_orders.py`, `api/routes_export.py` |
...
| 8.7 | **REFRAMED + SHIPPED (task 299)**: Manual-close reason modal (Q33). The spec's *trigger* — "on opposite-side order detection without calc match" — assumes a pre-close interception point that the observe-only engine does NOT have (closes are seen post-arrival via WS, same constraint as §8.6). Reframed to a **post-arrival** flow: a manual close lands classified `MANUAL_OTHER` (`order_manager._determine_exit_reason`), which renders a **clickable reason badge** in the Position History close-reason cell; clicking opens an inline modal (dropdown of MANUAL_* subtypes + optional note) that POSTs to `update_close_reason`, and the refined reason/note survive a later close-row REPLACE rebuild (preserve logic in `insert_closed_position`). **NO `static/js/manual_close_modal.js`** — the trigger is a badge click, not order-stream detection. | `templates/fragments/history/close_reason_badge.html` + `close_reason_cell.html`, `core/db_orders.py` (`update_close_reason` + REPLACE-preserve), `api/routes_history.py` |
...
| P4.T5 | **SHIPPED** — TP/SL drift columns at close | `tp_drift_pct`/`sl_drift_pct` = final amended TP/SL (latest `order_amendments`) vs the PRIMARY calc's planned (§3.2 basis); amended-only → NULL; persisted + REPLACE-preserved. Audit fix: orphaned pre-link amendment backfill in `_propagate_bracket_calc_id`. See detailed §4 row 4.6. **Phase 4 plan tasks T1–T5 all shipped.** |
...
| P7.T2 | **SHIPPED (task 271)** — `GET /context/position/{id}` | Position-keyed graph via `assemble_position_context` (pivots on `terminal_position_id`); shared `_aggregate_tail` with lifecycle; works for junction-less positions; 2 new position reads. Audited; 38 tests. See detailed §7 row 7.2. |
| P7.T3 | **SHIPPED (tasks 274-275)** — Webhook dispatcher + retry + dead-letter | First event_bus `position:closed` subscriber; per-account `webhook_url` (STRICT-bool flag, default OFF); enqueue→worker→POST with backoff; engine_events dead-letter; `start_webhook_dispatcher` wiring. Audited; 19 tests. See detailed §7 rows 7.4+7.5. |
| P7.T4 | **SHIPPED (tasks 277-278)** — Audit export — JSON only | `POST /export/closed_position/{id}` → signed JSON bundle (reuses the P7.2 assembler + a SHA-256/HMAC signed-timestamp envelope). Audited; 15 tests. See detailed §7 row 7.6. |
| P7.T5 | **SHIPPED (tasks 280-281)** — PDF generation + signed timestamp | `render_export_pdf` + a VENDORED dependency-free `core/pdf_writer.py` (operator chose no dependency); `?format=pdf` on the export route; same signature in a footer. PDF byte-validity audited; 26 tests. See detailed §7 row 7.6. |
| P7.T6 | **SHIPPED (tasks 283-284)** — Batch export endpoint | `POST /export/closed_positions` → ZIP of signed per-position bundles + a signed manifest; dedup by tpid; cap-flagged; account-scoped. Audited; 38 tests. **Phase 7 COMPLETE.** See detailed §7 row 7.7. |
...
| P8.T6 | Manual-close reason modal | **REFRAMED (task 299)**: post-arrival — clickable reason badge on the history close-reason cell (NOT opposite-side order-stream detection, impossible in observe-only) → MANUAL_* dropdown + note → REPLACE-preserved. See §8 detailed row 8.7. |

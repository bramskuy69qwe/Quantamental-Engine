// ════════════════════════════════════════════════════════════════════════
//  QE v2.5 — Notification System (integrated)
...
function readHaltUntil() { try { const v = +localStorage.getItem(HALT_KEY); return v && v > Date.now() ? v : 0; } catch (e) { return 0; } }
function readHaltAt() {
  try {
    const stored = +localStorage.getItem(HALT_AT_KEY);
    if (stored) return stored;
    // backfill: an active halt with no recorded start (legacy) — stamp it now so the banner shows a time
    const until = +localStorage.getItem(HALT_KEY);
    if (until && until > Date.now()) { const now = Date.now(); localStorage.setItem(HALT_AT_KEY, String(now)); return now; }
    return 0;
  } catch (e) { return 0; }
}
...
function NotificationProvider({ children, demo = true }) {
  ...
  const toggleDesktop = () => { if (!desktop && 'Notification' in window && Notification.permission === 'default') { try { Notification.requestPermission(); } catch(e){} } setDesktop(v => !v); };

/* QE v2.5 — Shared mock data & nav primitives used across all dashboard variants */
...
// ── One mock clock for the whole app ─────────────────────────────────
// Anchored to the engine's frozen "now" (= MOCK.exchange.serverTime) and
// ticking forward from page load. EVERY clock/timestamp surface renders
// through this offset so the nav clock, footer clock, notifications and
// engine log all agree (no real-wall-clock leakage).
const QE_CLOCK_OFFSET = Date.parse('2026-04-25T20:08:36Z') - Date.now();
const qeMockClockFmt = (t) => { const s = new Date(t.getTime() + QE_CLOCK_OFFSET).toISOString(); return s.slice(0, 10) + ' ' + s.slice(11, 19) + ' UTC'; };

// ── Shared live metrics — ONE random-walk ticker per metric id ──────────
// Every surface that shows ws-latency / P&L reads the SAME value (was:
// independent per-component walks disagreeing on-screen at the same moment).
...
const TopNavStd = ({page='Dashboard', onChange, variant='line', dense=false}) => {
  ...
  <div className="qe-hscroll" style={{
    ...
    <div style={{padding:'0 14px', display:'flex', alignItems:'baseline', gap:8, borderRight:'1px solid var(--qe-line)', alignSelf:'stretch', alignItems:'center'}}>
      <span style={{fontFamily:'var(--qe-mono)', fontSize:'0.7rem', fontWeight:700, color:'var(--qe-cyan)', letterSpacing:'0.08em'}}>MERIDIAN</span>
      <span style={{fontFamily:'var(--qe-mono)', fontSize:'0.56rem', color:'var(--qe-sub)', letterSpacing:'0.06em'}}>v3.0</span>
    </div>
    ...
    <div style={{display:'flex', alignItems:'center', gap:8, padding:'0 12px', alignSelf:'stretch'}}>
      ...
      <span style={{
        ...
        letterSpacing:'0.04em',
      }}><LiveClock id="nav.clock" format={qeMockClockFmt}/></span>
...
const StatusFooter = () => {
  ...
  return (
    <div style={{
      ...
      <span style={{color:'var(--qe-sub)'}}>MERIDIAN v3.0</span>
      <span>·</span>
      ...
      <LiveClock id="sb.clock" format={qeMockClockFmt} style={{color:'var(--qe-muted)'}}/>
    </div>
  );
};

Object.assign(window, { MOCK, NAV_ITEMS, TopNavStd, WorkspaceBar, StatusFooter, MOCK_WATCHLIST, QE_CLOCK_OFFSET, qeMockClockFmt });

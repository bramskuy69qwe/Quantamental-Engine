...
**Status**: **IMPLEMENTED — program COMPLETE (Phases 0–5, 2026-06-14)**; design rev 2 audited (4 adversarial agents, 2026-06-10). As-built deviations in §15 (E1–E37; E35–E36 filed by the linkage-battery + attribution-reconciler program, 2026-07-15 — NB §12's `attr_decide` consolidation was superseded by E36's outcome-riding; **E37 filed by the v2.6 audit, 2026-07-16 — the Quantower plugin surface is REMOVED from §3.2/§4/§5.4 and the registry is 42, not 46**).
**Created**: 2026-06-10
...
| `component` | str | The subsystem emitting. Closed-ish set: `http`, `ws_manager`, `news_fetcher`, `order_manager`, `position_identity`, `data_cache`, `calc_state`, `link_state`, `adapters.binance` (`.bybit`/`.mexc`), `event_bus`, `pubsub`, `scheduler`, `webhook_dispatcher`, `funding_handler`, `reconciler`, `db`. (`platform_bridge` removed — v2.6/E37.) |
| `peer` | str | The other side of the boundary. `binance`, `operator` (browser), `finnhub`, `fred`, `yahoo`, `bwenews`, `subscriber` (webhook receiver), `event_bus`, `internal`, `disk`. Every peer listed here is **reachable by at least one §5 category** (audit rule: no dead peers). (`quantower` removed — v2.6/E37 dropped its four producer-less categories; it was the last peer this rule failed on.) |
...
(v2.6/E37: the `platform_bridge` / `quantower` row — `platform_fill` /
`platform_snapshot` / `platform_hello` — and the outbound `platform_push`
mirror category were REMOVED. The Quantower plugin was their only producer and
v2.6 Phase 5 deleted it.)
...
|---|---|
| `ws_connect` | stream (user/market/news — `platform` removed with the plugin, v2.6/E37; NB E5 already narrowed this enum), **full subscribed-stream list**, attempt# |
...
| `data_cache` | `account_update_applied` | source (rest/platform — the `platform` value is now UNREACHABLE, not removed: `data_cache.apply_account_update_platform` survives v2.6 as a deliberate landmine (plan L3) but has zero callers, so it can no longer emit), equity before/after, waited_ms/held_ms |
...
1. For any single inbound frame (Binance WS, news WS — v2.6/E37 removed the
   platform WS frame class with the plugin), HTTP
...
| E34 | §5.5, §7.3 | **HA-24 ACCEPTED**: platform `account_update_applied` stays per-frame (~5 Hz when the Quantower plugin streams), NOT gated/sampled | the plugin is **not connected at this deployment** (Binance-direct); the per-frame cost is latent and bounded by the per-day MB guard; gating a STATE-group category per-source adds complexity for an inactive path (CLAUDE.md "don't build speculative"). ~~Re-elevate when the Quantower plugin is wired~~ — **SUPERSEDED / CLOSED-not-applicable by E37 (v2.6, 2026-07-16)**: the plugin was deleted, so the ~5 Hz platform lane E34 declined to gate cannot exist and the re-elevation trigger can never fire. `account_update_applied` remains per-frame on the Binance-direct lane, which is what E34's cost reasoning already assumed |
...
| E36 | §12 (:882), §5.6-close (:490), §3, §5.6 | **The attribution reconciler shipped (R1–R5, 2026-07-15; `core/position_identity.py`) with the TEN `attr_*` categories KEPT verbatim and NO new category** — neither §12's "single `attr_decide`" consolidation NOR the reconciler plan §2.3's proposed `attr_identity_reconcile`. The owner's genuinely-new decisions ride **`attr_junction_form` as outcome-vocabulary additions**: `MIGRATED` (R2 key migration — one line per migrated row, post-commit), `RECONCILED` (R3 delta-reconcile — on-change only, carries old/new qty + swept-fill count), `SEALED` (R4 lifecycle seal-at-close — emitted only when rows actually sealed; dedup_key `seal:{terminal_position_id}:{sealed_ts}`). The registry stays **46** categories [**count SUPERSEDED by E37: 42** — v2.6 removed the four producer-less `platform_*` categories. E36's *reasoning* is untouched: it argued against ADDING a category, and "registry churn is expensive" is why the reconciler rode existing outcomes. Deleting four categories with no producer is not that trade]. §3's `component` set gains **`position_identity`**: the moved decision sites (`attr_tpid_resolve` close-fill lane, `attr_junction_form` incl. the replay lane) now emit with it — NB `attr_tpid_resolve` is MIXED-component: the E26 `via=snapshot_recovery` lane did NOT move and stays `order_manager` — while `attr_close_stamp`/`attr_enrich`/`attr_drift_check` stay `order_manager` and `attr_match_attempt`/`attr_bracket_inherit`/`attr_reenrich_trigger`/`attr_funding_assign` keep their original owning modules (§5.6 site column read accordingly) | Deviation-discipline record — "outcome-riding INSTEAD OF a new category BECAUSE": (1) category-name stability is what §12's faithful-consolidation proof runs on (pre/post `attr_*` diffs compare like-for-like; R1 shipped envelope-equivalent modulo `component`); (2) migration/reconcile/seal are junction-ROW mutations inside the formation flow — same identity tuple, same dedup_key family, same corr chain as their FORMED/SKIPPED siblings; a separate category would split one decision flow across two greps; (3) no tooling filters between attr categories (cookbook + `corr_tail.py` verified — `attr_*` and per-category greps both keep working); (4) outcome churn is cheaper than registry churn (the 46-category registry is snapshot-pinned; profiles/tests unchanged) |
| E37 | §3.2, §4, §5.4, §7.3, E34, E36 | **v2.6 removed the Quantower plugin (2026-07-15, `aeb397f`) — the spec's plugin surface is DELETED, not deprecated-in-place.** Dropped: §3.2's `wsp` entry-point row (`core/platform_bridge.py` via `/ws/platform`, which this table called "**Money-path critical**"); `platform_bridge` from §4's `component` set; `quantower` from §4's `peer` set; §5.4's `platform_fill` / `platform_snapshot` / `platform_hello` row + the outbound `platform_push` mirror; the four `register()` calls in `core/correlation_log.py`. **Registry 46 → 42** (`test_registry_snapshot_ha3` updated in the same commit — the pin and the registry MUST move together). §4's "**no dead peers**" audit rule HOLDS AGAIN with no carve-out: `quantower` was the only peer it had come to fail on. E34 closed not-applicable (its "re-elevate when the plugin is wired" trigger can never fire); E36's `46` superseded to `42`. §5.4's HA-9 `platform_snapshot` payload gap is moot. Cookbook's `platform_fill` dedup-key row removed. **Not merely stale prose — §3.2 is the table the log's own audit rule reads against** ("a `corr_id=""`/fallback envelope is itself a finding that an entry point is missing a scope"), so leaving it would have sent the next auditor hunting a `wsp` scope that cannot exist | Filed by the v2.6 holistic audit (2026-07-16) as its only HIGH: P6's doc sweep covered README/config.md/CLAUDE.md/AGENTS.md/binance.md but **never opened this file**, so the spec kept describing a deleted module as a live money path. **PURGE INSTEAD OF mark-producer-less BECAUSE** the honest alternative was carving a `quantower` exception into a sound audit rule to accommodate rot — and these categories are unreachable *forever*: v2.6 deleted their only producer and is permanent, so none can be re-emitted (the deleted source is recoverable from git history, and additionally from the LOCAL-only branch `archive/quantower-plugin` at pre-deletion HEAD `523e0d1` — local-only, so treat git history as the durable record). Blast radius verified small before committing: no `cl.emit(…CAT_PLATFORM_*)` exists anywhere, and no test asserts on them beyond the registry snapshot. **Errata that mention the plugin surface — E2 (the `platform_push` market-grouping rationale), E4, E5, E6 (component example), E12, E18 (import-style note) — are LEFT VERBATIM**: errata are a dated record of what was true when filed, and rewriting them would destroy the audit trail. This entry supersedes them. NB E2 remains the home of the `platform_push` GROUP_MARKET grouping rationale, which is why the deleted code comment's reasoning is not lost |
...
kind only~~ (MOOT — category removed, v2.6/E37); `ws_depth` lacks
top-of-book. **Phase-2 add (T2b)**: the
